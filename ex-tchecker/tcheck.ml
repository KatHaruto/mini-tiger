(* A type checker, which takes an expression of type Absyn.exp as an input,
   and outputs an expression of type Absyn2.exp.
   In addition to type checking, variables are renamed so that each variable
   has a unique name. 
 *)
(* In this file, the case for function declarations is left for an exercise *)
open Absyn
exception TODO

(* an environment that maps a variable v to a pair (v',vty), where
   v' is the unique name of v, and vty is the type of v *)
...
assoc a [ ...; (a,b); ...] = b if (a,b) is the leftmost binding of a in list l.
...
and tcheck_dec dec tenv =
 ...
 | FunctionDec(f, args, tyopt, exp) ->
     ...
     let tenv' = extend_tenv_vs tenv argenv  in
     let (exp',ty) = tcheck exp tenv' in 
     let _ = match tyopt with
                Some(ty') -> 
                if ty=ty' then () 
            ...
            | _ -> ()
     in
     let fty = get_fty args tyopt in 
     let tenv' =  extend_tenv_f f fty tenv in
     let (f',_) = lookup_fun f tenv' in
     let arg' = List.map( fun arg -> Absyn2.IfExp(name_of_sym arg)) args in
        (Absyn2.FunctionDec(f',arg', ty, exp'), tenv')
